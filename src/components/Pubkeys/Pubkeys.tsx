...
import { type PubKeyReason, banPubkey, loadWhitelist, whitelistPubkey } from "./api";
...
export default function Pubkeys() {
    ...
    return (
        <>
            ...
            <ul>
                {whitelist.sort((a, b) => ('' + a.pubkey).localeCompare(b.pubkey)).map(wl => {
                    return (
                        <li className="whitelist" key={wl.pubkey}>
                            {wl.pubkey}
                            <span className="whitelist-remove" onClick={() => handleBanClick(wl.pubkey)}>❌</span>
                        </li>
                    )
                })}
            </ul>
